...
export default function App() {
  ...
  return <div className="min-h-screen bg-[#f5f6fa] text-[#1c075c]">
    ...
    <main className="min-h-screen lg:pl-72">
      ...
      {page === "dashboard" && currentStaff?.role !== "Kasir" && <header className="sticky top-0 z-30 flex h-14 items-center justify-between bg-transparent px-5 lg:hidden"><button onClick={() => setSidebarOpen(true)} aria-label="Buka menu" className="grid size-10 place-items-center rounded-2xl bg-white shadow-sm ring-1 ring-gray-100"><Menu size={18} className="text-gray-600"/></button><button onClick={() => currentStaff ? go("profile") : setLoginOpen(true)} aria-label={currentStaff ? "Buka profil" : "Login akun"} className="grid size-10 place-items-center overflow-hidden rounded-full bg-gradient-to-br from-amber-100 to-rose-300 text-lg shadow-sm ring-1 ring-gray-100">{currentStaff?.photo?<img src={currentStaff.photo} alt="Profil" className="size-full object-cover"/>:"👩"}</button></header>}
      {page === "kitchen" && <KitchenDisplay transactions={transactions} onRefresh={() => window.location.reload()} />}
